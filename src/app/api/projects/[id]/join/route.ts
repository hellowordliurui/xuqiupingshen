 ...
 * 准入逻辑见 背景/需求辩论-对话流程逻辑.md：人数硬连接（当前房间人数 < MAX_ROOM_SIZE）。
 * 当前实现：校验指定角色席位存在且为空、本人未重复加入；可选扩展为「不传 role 时占任意空位」。
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const card = updated
    ...
    : null;
  return NextResponse.json({ code: 0, data: card, message: "已加入辩论" });
}
